...
import { Check, Info, ListFilter, Plus, Search, X } from "lucide-react";

function normalize(value) {
  return String(value || "").trim().toLocaleLowerCase();
}

export default function AssistPreferences() {
  ...
  const addKeyword = (event) => {
    ...
    const duplicate = keywords.some(
      (keyword) => normalize(keyword) === normalize(value)
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
        ...
        <div>
          This is a UI preview only. Keywords are not saved or sent to the backend yet.
        </div>
      </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
                When this list has keywords, Assist will process a message only when it contains at least one of them.
              </p>
            </div>
          </div>
          <span
        ...
        <form onSubmit={addKeyword} className="mt-5 flex flex-col gap-2 sm:flex-row">
          <input
            type="text"
            value={draft}
        ...
        <div className="mt-4 min-h-12 rounded-lg border border-dashed border-slate-200 bg-slate-50/70 p-3">
          {keywords.length === 0 ? (
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        <div className="flex gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-slate-900">Try a message</h2>
            <p className="mt-1 text-sm text-slate-500">
              Test how the keyword filter would behave before we connect it to the backend.
            </p>
          </div>
        </div>
        ...
        <textarea
          ...
          className="mt-4 w-full resize-y rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200/60"
        />
